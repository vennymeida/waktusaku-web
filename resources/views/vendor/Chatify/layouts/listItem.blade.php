...
@if ($get == 'saved')
    <table class="messenger-list-item" data-contact="{{ Auth::user()->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ Auth::user()->id }}" data-type="user">Pesan Tersimpan <span>Kamu</span></p>
        ...
        <tr data-action="0">
            ...
            <td>
                <div class="avatar av-m"
                    @if ($user->profile && $user->profile->foto) style="background-image: url('{{ asset('storage/' . $user->profile->foto) }}');"
        @else
        style="background-image: url('{{ $user->avatar }}');"> @endif
                    </div>
            </td>
            {{-- center side --}}
            <td>
